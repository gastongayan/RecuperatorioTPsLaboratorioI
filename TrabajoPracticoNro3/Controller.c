...
int controller_editEmployee(LinkedList* pArrayListEmployee)
{
	...
	if(pArrayListEmployee != NULL)
	{
		if(!ll_isEmpty(pArrayListEmployee)){
			...
			if(!CargarUnEntero("Desea mostrar la lista de empleados? (1=SI 0=NO)",&option, 0, 1)){
				if(option == 1)
				{
			...
			CargarUnEntero("Ingrese id del empleado que desea modificar:\n",&id, 0, idMayor);
			...
			if(index!=-1)
			{
				...
				do{
					if(!CargarUnEntero("Ingrese campo a modificar\n1. Nombre\n2.Horas trabajadas\n3. Sueldo\n0. Salir",&option, 0, 3)){
						switch(option)
						{
							case 1:
							...
							case 2:
								CargarUnEntero("Ingrese cantidad de horas trabajadas",&(auxEmpleado->horasTrabajadas), 0, 1000);
								flag = 1;
								break;

							case 3:
								CargarUnEntero("Ingrese sueldo:",&(auxEmpleado->sueldo), 0, 999999);
								flag = 1;
								break;
				...
				if(flag == 1){
					if(!CargarUnEntero("Desea guardar los cambios realizados? (1=SI 0=NO)",&option, 0, 1)){
						rtn = 1;
						pEmpleado = auxEmpleado;
					}
				}
			}else{
				if(CargarUnEntero("Dato no encontrado\nDesea continuar buscando? (1=SI 0=NO)",&option, 0, 1))
				{
					if(option==1){
	...
	if(pArrayListEmployee != NULL)
		{
			if(!ll_isEmpty(pArrayListEmployee)){
				...
				idMayor = controller_mayorId(pArrayListEmployee);
				if(!CargarUnEntero("Ingrese id del empleado que desea eliminar:\n",&id, 0, idMayor)){
					index = employee_getIndexById(pArrayListEmployee, id);
					if(index != -1){
						pEmpleado = (Employee*) ll_get(pArrayListEmployee, index);
						if(pEmpleado!=NULL)
						{
							...
							if(!CargarUnEntero("\nEsta seguro que desea eliminar el empleado seleccionado? (1=SI 0=NO)", &option, 0, 1)){
								if(option==1)
								{
...
int controller_sortEmployee(LinkedList* pArrayListEmployee)
{
	...
	if(pArrayListEmployee!=NULL)
	{
		if(!ll_isEmpty(pArrayListEmployee))
		{
			...
			if(!CargarUnEntero("Ingrese campo por el que desea ordenar:\n1-Id\n2-Nombre\n3-Horas Trabajadas\n4-Sueldo\n0-Salir\n", &option, 0, 4) &&
					!CargarUnEntero("Ingrese criterio de ordenacion: (1-Ascendente 0-Descendente):\n",&criterio, 0, 1)){

				if(option!=-1 && criterio!= -1)
				{
					switch(option){
